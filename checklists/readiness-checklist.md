...
```
- - Đã khắc phục lỗi xung đột tên container bằng lệnh 'docker rm -f'.
- Đã cấu hình lại Healthcheck trong docker-compose.yml bằng 'python -c' để thay thế 'curl' 
  nhằm tương thích với base image.
- Đã ép 'ai-service' chạy đúng command 'uvicorn ai_service.main:app' trên port 9000.
- Hệ thống đạt trạng thái 'healthy' cho tất cả các service sau khi cấu hình lại.
```